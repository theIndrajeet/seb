<template>
  <div class="process-page">
    <!-- 顶部导航栏 -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">SEBASTIAN</div>
...
<style scoped>
/* 变量 — mapped to Sebastian design tokens */
:root {
  --black: var(--sb-text);
  --white: transparent;
  --orange: var(--sb-violet);
  --gray-light: rgba(255, 255, 255, 0.06);
  --gray-border: var(--sb-glass-border);
  --gray-text: var(--sb-text-secondary);
}
...
.left-panel {
  ...
  transition: width 0.35s var(--sb-ease);
  background: transparent;
  z-index: 5;
}
...
.loading-ring:nth-child(2) {
  ...
  top: 10px;
  left: 10px;
  border-right-color: var(--sb-indigo);
...
.right-panel {
  ...
  flex-direction: column;
  background: transparent;
  transition: width 0.35s var(--sb-ease), opacity 0.3s ease, transform 0.3s ease;
  overflow: hidden;
  opacity: 1;
}
...
.waiting-state {
  padding: 16px;
  background: rgba(255, 255, 255, 0.04);
  border: 1px dashed var(--sb-glass-border);
  border-radius: var(--sb-radius-sm);
  text-align: center;
}
...
.next-step-btn {
  ...
  letter-spacing: 0.05em;
  cursor: pointer;
  transition: transform 0.2s var(--sb-ease), box-shadow 0.25s var(--sb-ease), opacity 0.2s ease;
}
